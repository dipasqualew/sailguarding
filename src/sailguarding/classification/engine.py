...
3. If, after 1 and 2, the top rules still bind to *different* activities, the classification is
...
Top rules that all name the *same* activity are not a conflict: the event simply resolves to it.
"""
...
class SelectorClassificationStrategy:
    ...
    def classify(self, event: EventRecord) -> Classification:
        ...
        activity_ids = {rule.activity_id for rule in top}
        if len(activity_ids) == 1:
            return Classification.matched(next(iter(activity_ids)))
        return Classification.ambiguous(tuple(sorted(activity_ids)))
